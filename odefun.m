    ...
    for j=1:N
        if freq(i) < freq(j)
            dP(i) = dP(i) + P(i)*(gain(freq(i)/1e13)/Aeff)*P(j)...
                +2*h*freq(i)*P(j)*(1+1/exp(h*abs(freq(i)-freq(j))/(k*T)-1))*(freq(j)-freq(i)) ...
                -4*h*freq(i)*P(i)*(gain(freq(i)/1e13)/Aeff)*(1+1/exp(h*abs(freq(i)-freq(j))/(k*T)-1))*(freq(j)-freq(i));
        else
            dP(i) = dP(i) - P(i)*(freq(i)/freq(j))*(gain(freq(j)/1e13)/Aeff)*P(j)...
                -4*h*freq(i)*P(i)*(1+1/exp(h*abs(freq(i)-freq(j))/(k*T)-1))*abs(freq(i)-freq(j))...
                +2*h*freq(i)*P(j)*(gain(freq(i)/1e13)/Aeff)*(1/exp(h*abs(freq(i)-freq(j))/(k*T)-1))*(freq(j)-freq(i));
        end
    end
end
end